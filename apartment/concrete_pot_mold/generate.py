...
parser.add_argument('--bottom', type=float, default=1.2, help="Mold bottom surface thickness")
parser.add_argument('--side-holes', type=int, default=0, help="Add holes for joining mold halves")
...
if args.side_holes != 0:
    print("    intersection() {")
    print("      difference() {")
    print("        scale([{0:.6f}, {0:.6f}, 1]) {{".format(scale_factor(7)))
    print("          main_shape();")
    print("        }")
    print("        scale([{0:.6f}, {0:.6f}, 1]) {{".format(scale_factor(4)))
    print("          main_shape();")
    print("        }")
    print("      }")
    print("      h1=6;")
    print("      h2=0.4*h1;")
    print("      for(h=[0:2*h1:{0:.2f}]) {{".format(args.height))
    print("        translate([0, 0, h]) {")
    print("          cylinder(d=4*{0:.2f}, h=h2, $fn=6);".format(mold_hold_distance, args.wall))
    print("        }")
    print("      }")
    print("    }")
print("  }")
print("  difference() {")
print("    translate([0, 0, -{0:.2f}]) {{".format(args.bottom))
print("      cube([10*{0:.2f}, {1:.2f}, {0:.2f}]);".format(args.bottom, mold_hold_distance))
print("    }")
print("    mirror([0, 0, 1]) {")
print("      main_shape();")
print("    }")
...
print("          main_shape();")
print("        }")
print("        cube([2*{0:.2f}, 2*{0:.2f}, 2*{1:.2f}], center=true);".format(mold_hold_distance, args.bottom))
print("      }")
print("      mirror([1, 0, 0]) {")
print("        translate([0, -{0:.2f}, 0]) {{".format(mold_hold_distance))
print("          cube([{0:.2f}, 2*{0:.2f}, {1:.2f}]);".format(mold_hold_distance, args.height))
print("        }")
print("      }")
print("      cylinder(h=2*{0:.2f}, d={1:.2f}, $fn=100);".format(args.bottom, args.hole_diameter))
print("    }")
print("  }")
print("}")
print("")
